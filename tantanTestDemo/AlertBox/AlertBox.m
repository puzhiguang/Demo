...
+ (void)removeBoxes {
    UIWindow *window = [[self class] rootWindow];
...
- (id)initWithTitle: (NSString *)title WithText: (NSString *)text fontSize:(int)fontSize icon:(UIImage *)icon {
    ...
    if (self) {
        ...
        _mask = [[UIView alloc] initWithFrame:[[self class] rootWindow].bounds];
        _mask.backgroundColor = [UIColor blackColor];
        _mask.alpha = .3f;
        _mask.hidden = YES;
        UITapGestureRecognizer*tapGesture = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(hide)];
        [_mask setGestureRecognizers:@[tapGesture]];
        [[[self class] rootWindow] addSubview:_mask];
        [self layout];
    }
    return self;
}
...
- (void)layout {
    ...
    const float MAX_FRAME_WIDTH = 225.0/375.0 * [Meilishuo screenWidth];
    ...
    if ([Meilishuo getSystemVersion] < 5.0) {
        self.frame = CGRectMake(([Meilishuo screenWidth] - frameWidth)/2,
                               ([Meilishuo screenHeightWithoutStatusBar] - frameHeight)/2,
                               frameWidth,
                               frameHeight);   
    } else if([Meilishuo getSystemVersion] < 6.0){
        self.frame = CGRectMake(([Meilishuo screenWidth] - frameWidth)/2,
                               ([Meilishuo screenHeight] - frameHeight)/2,
                               frameWidth,
                               frameHeight);
    }
    else{
        self.frame = CGRectMake(([Meilishuo screenWidth] - frameWidth)/2,
                                ([Meilishuo screenHeight] - frameHeight)/2-44,
                                frameWidth,
                                frameHeight);
...
- (void)show {
    ...
    [[[self class] rootWindow] addSubview:self];
}
